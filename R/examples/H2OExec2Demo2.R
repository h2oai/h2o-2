...
airlines.qs = quantile(airlines.hex$ArrDelay)
print(airlines.qs)

# Note: Right now, assignment must be done manually with h2o.assign!
outliers.ind = airlines.hex$ArrDelay <= airlines.qs[2] | airlines.hex$ArrDelay >= airlines.qs[10]
...
s = h2o.runif(airlines.hex)    # Use if number of rows too large for R to handle

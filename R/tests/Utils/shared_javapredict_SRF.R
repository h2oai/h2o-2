...
iris.rf.h2o <- h2o.randomForest(x = x, y = y, data = iris_train.hex, ntree = ntree, depth = depth, nodesize = nodesize, balance.classes = balance_classes, seed = 42, type = "fast")
print(iris.rf.h2o)
...
safeSystem(cmd)
cmd <- sprintf("curl -o %s/%s.java http://%s:%d/2/SpeeDRFModelView.java?_modelKey=%s", tmpdir_name, model_key, myIP, myPort, model_key)
safeSystem(cmd)

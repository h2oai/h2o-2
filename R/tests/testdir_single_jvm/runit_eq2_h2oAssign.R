...
test.eq2.h2o.assign<-
function(conn) {
    iris.hex <- h2o.uploadFile(conn, "~/master/h2o/smalldata/iris/iris.csv", "iris.hex")
    dim(iris.hex)
    ...
    expect_that(length(keyList), equals(2))
    expect_that(keyList$ByteSize[1], equals(keyList$ByteSize[2]))
    iris.hex <- h2o.assign(iris.hex, "iris.hex")

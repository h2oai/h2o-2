...
test.glm2linear.numeric <- function(H2Oserver) {
  Log.info("\nImporting swiss.csv data...\n")
  swiss.hex<- h2o.importFile(H2Oserver, "./smalldata/swiss.csv")
  swiss.df<- read.csv("../../smalldata/swiss.csv", header=T)
...
#check produced values against known values
  Log.info("\nCheck that the descriptives from H2O matches known good values... ")
  library(testthat)
  ...
  expect_that(swiss.h2o@model$df.residual, equals(41))
  expect_that(swiss.h2o@model$df.null, equals(46))
  expect_true(abs(swiss.h2o@model$aic-326)<(1))  

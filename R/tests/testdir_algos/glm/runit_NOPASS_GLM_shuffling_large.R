...
test <- function(conn) {
    print("Reading in Arcene training data for binomial modeling.")
        arcene.train.full = h2o.uploadFile(conn, locate("smalldata/arcene/arcene_train.data"), key="arcene.train.full", header=FALSE)
        arcene.train = h2o.assign(arcene.train.full[,c(1:1000)],"arcene.train")
    ...
    print("Create model on original Arcene dataset.")
        h2o.model <- h2o.glm(x=c(1:1000), y="arcene.train.label", data=arcene.train.full, family="binomial", lambda_search=TRUE, alpha=0.5, nfolds=0, use_all_factor_levels=1)

    print("Create second model on original Arcene dataset.")
        h2o.model2 <- h2o.glm(x=c(1:1000), y="arcene.train.label", data=arcene.train.full, family="binomial", lambda_search=TRUE, alpha=0.5, nfolds=0, use_all_factor_levels=1)

    print("Create model on shuffled Arcene dataset.")
        h2o.model.s <- h2o.glm(x=c(1:1000), y="arcene.train.label", data=arcene.train.full2, family="binomial", lambda_search=TRUE, alpha=0.5, nfolds=0, use_all_factor_levels=1)

...
test <- function(conn) {
    print("Generate balanced dataset by column in R")
        ...
        x1.a = sample(-1:-10, 200, replace=T)
        x1.b = runif(200, 5.0, 20.0)
        x2.a = sample(6:10, 200, replace=T)
        x2.b = runif(200, -6.0, 2.0)
    ...
    print("Fit model on dataset.")
        model.balanced <- h2o.glm(x=c("x1", "x2"), y="y", data.b.hex, family="binomial", lambda_search=TRUE, use_all_factor_levels=1, alpha=0.5, nfolds=0, higher_accuracy=T, lambda=0)
    ...
    print("Fit model on dataset.")
        model.unbalanced <- h2o.glm(x=c("x1", "x2"), y="y", data.u.hex, family="binomial", lambda_search=TRUE, use_all_factor_levels=1, alpha=0.5, nfolds=0, higher_accuracy=T, lambda=0)

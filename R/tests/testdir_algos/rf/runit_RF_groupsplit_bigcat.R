...
test.DRF.bigcat <- function(conn) {
  ...
  drfmodel.nogrp <- h2o.randomForest(x = "X", y = "y", data = bigcat.hex, classification = TRUE, ntree = 1, depth = 1, nbins = 100, doGrpSplit = FALSE)
  print(drfmodel.nogrp)
  
  Log.info("H2O DRF (Group Split) with parameters:\nclassification = TRUE, ntree = 1, depth = 1, nbins = 100\n")
  drfmodel.grpsplit <- h2o.randomForest(x = "X", y = "y", data = bigcat.hex, classification = TRUE, ntree = 1, depth = 1, nbins = 100, doGrpSplit = TRUE)

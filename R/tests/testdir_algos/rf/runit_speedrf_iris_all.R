...
test.speedrf.iris_class <- function(conn) {
  ...
  iris.rf  <- h2o.SpeeDRF(y = 5, x = 1:4, data = iris.hex, ntree = 50, depth = 100, importance = TRUE) 
  print(iris.rf)
  iris.rf  <- h2o.SpeeDRF(y = 6, x = 1:4, data = iris.hex, ntree = 50, depth = 100, importance = TRUE )
  print(iris.rf)
  testEnd()
}

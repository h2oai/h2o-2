...
test.speedrf.airlines.twoing<- function(conn) {
  ...
  air.rf <- h2o.randomForest(y= 'IsDepDelayed' , x = c('Year', 'Month', 'DayofMonth', 'DayOfWeek', 'CRSDepTime', 'CRSArrTime', 'UniqueCarrier', 'CRSElapsedTime', 'Origin', 'Dest', 'Distance') , data = air.hex , ntree = 50, stat.type="TWOING", depth = 20, oobee = TRUE, importance = FALSE, nfolds = 0, sample.rate = 0.67, mtries = -1) 

...
test.speedrf.airlines.twoing.fullsamplerate<- function(conn) {
  ...
  air.rf <- h2o.randomForest(y= 'IsDepDelayed' , x = c('Year', 'Month', 'DayofMonth', 'DayOfWeek', 'CRSDepTime', 'CRSArrTime', 'UniqueCarrier', 'CRSElapsedTime', 'Origin', 'Dest', 'Distance') , data = air.hex , ntree = 50, stat.type="TWOING", depth = 20, oobee = FALSE, importance = FALSE, nfolds = 0, sample.rate = 1.00, mtries = -1, validation=air.hex, seed = 11) 
...
doTest("speedrf test air twoing", test.speedrf.airlines.twoing.fullsamplerate)

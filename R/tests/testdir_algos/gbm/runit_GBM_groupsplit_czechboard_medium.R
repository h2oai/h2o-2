...
test.GBM.Czechboard <- function(conn) {
  ...
  drfmodel.nogrp <- h2o.gbm(x = c("C1", "C2"), y = "C3", data = board.hex, n.trees = 50, interaction.depth = 20, n.bins = 500, group_split = FALSE)
  print(drfmodel.nogrp)
  
  Log.info("H2O GBM (Group Split) with parameters:\nclassification = TRUE, n.trees = 50, interaction.depth = 20, n.bins = 500\n")
  drfmodel.grpsplit <- h2o.gbm(x = c("C1", "C2"), y = "C3", data = board.hex, n.trees = 50, interaction.depth = 20, n.bins = 500, group_split = TRUE)

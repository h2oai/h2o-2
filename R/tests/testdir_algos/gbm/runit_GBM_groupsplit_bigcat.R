...
test.GBM.bigcat <- function(conn) {
  ...
  drfmodel.nogrp <- h2o.gbm(x = "X", y = "y", data = bigcat.hex, n.trees = 1, interaction.depth = 1, n.bins = 100, group_split = FALSE)
  print(drfmodel.nogrp)
  
  Log.info("H2O GBM (Group Split) with parameters:\nclassification = TRUE, n.trees = 1, interaction.depth = 1, n.bins = 100\n")
  drfmodel.grpsplit <- h2o.gbm(x = "X", y = "y", data = bigcat.hex, n.trees = 1, interaction.depth = 1, n.bins = 100, group_split = TRUE)

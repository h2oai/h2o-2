            ...
            simpleFilterTest_test_32 <- function(conn) {
                Log.info("A munge-task R unit test on data <test> testing the functional unit <!=> ")
                Log.info("Uploading test")
                hex <- h2o.uploadFile(conn, locate("../../smalldata/chess/chess_2x2x10/R/test.csv"), "rtest.hex")
            testEnd()
            }
            doTest("simpleFilterTest_ on data test unit= !=", simpleFilterTest_test_32)

...
test <- function(conn) {
	...
	h2o.glm.CV <- h2o.glm(x=myX, y=myY, data=mushroom.train, key="h2o.glm.CV.mushroom", family="binomial", alpha=1, higher_accuracy=T, lambda_search=T, nfolds=3,variable_importances=1, use_all_factor_levels=1)
	...
	myY = "C9"
	print("Creating model with CV")
	h2o.glm.CV <- h2o.glm(x=myX, y=myY, data=abalone.train, key="h2o.glm.CV.abalone", family="gaussian", alpha=1, higher_accuracy=T, lambda_search=T, nfolds=3, variable_importances=1, use_all_factor_levels=1)
	print(h2o.glm.CV)  #Confirm reported values accurate and match browser

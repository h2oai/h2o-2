...
test <- function(conn) {
  ...
  model.h2o.Gamma.inverse <- h2o.glm(x=myX, y=myY, data=prostate.data, family="gamma", link="inverse", alpha=1, lambda_search=T, variable_importance=1, use_all_factor_levels=1, nfolds=0)
  print(model.h2o.Gamma.inverse)      #AIC is NaN
  
  print("Testing for family: TWEEDIE")
  model.h2o.tweedie <- h2o.glm(x=myX, y=myY, data=prostate.data, family="tweedie", link="tweedie", alpha=1, lambda_search=T, variable_importance=1, use_all_factor_levels=1, nfolds=0)

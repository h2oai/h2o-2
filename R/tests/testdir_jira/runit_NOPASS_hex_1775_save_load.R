...
test.hex_1775 <- function(conn) {
  ...
  prostate.glm.path1 = h2o.saveModel(object = prostate.glm, dir = temp_dir, save_cv  = TRUE, force = TRUE)
 
  curdir <- getwd()
  print(curdir)
  prostate.glm.pathRSANDBOX <- h2o.saveModel(object = prostate.glm, filename = paste(curdir, "/Rsandbox_runit_hex_1775_save_load.R/runitHex1775_GLM.h2o", sep =""), save_cv = TRUE, force = TRUE)

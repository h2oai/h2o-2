...
adlt_income<-h2o.importFile(conn, normalizePath(locate("smalldata/adult.gz")),key="adlt_income")
myX = 1:14
myY = 15

print("Building SpeedRF model")
my.srf  = h2o.SpeeDRF(x=myX,y=myY,data=adlt_income,classification=T,ntree=50,oobee=F,seed = 11,validation=adlt_income)
print(my.srf)
...
mse_from_predFile = mean(c((1-ss[which(ss[,1]=="<=50K"),2])^2, (1-ss[which(ss[,1]==">50K"),3])^2, (1-tt[which(tt[,1]==">50K"),2])^2, (1-tt[which(tt[,1]=="<=50K"),3])^2))

print(paste("mean squared error from model page:  ", mse_from_model, sep = ''))
print(paste("mean squared error as calculated from predictions file:  ", mse_from_predFile, sep = ' '))
print("Expect the above two to be equal")
expect_equal(mse_from_model,mse_from_predFile)

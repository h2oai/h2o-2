...
test.summaryrunif.golden <- function(H2Oserver) {
...
AH2O<- h2o.uploadFile(H2Oserver, locate("../../smalldata/runifA.csv"), key="AH2O")
AR<- read.csv(locate("smalldata/runifA.csv"), header=T)
BH2O<- h2o.uploadFile(H2Oserver, locate("../../smalldata/runifB.csv"), key="BH2O")
BR<- read.csv(locate("smalldata/runifB.csv"), header=T)
CH2O<- h2o.uploadFile(H2Oserver, locate("../../smalldata/runifC.csv"), key="CH2O")
CR<- read.csv(locate("smalldata/runifC.csv"), header=T)
runifH2O<- h2o.uploadFile(H2Oserver, locate("../../smalldata/runif.csv"), key="CH2O")
...
doTest("Summary on Random Uniform Dist", test.summaryrunif.golden)

...
test.glm2glmnetreg3.golden <- function(H2Oserver) {
...
fitRglmnet<- glmnet(x, y, family="gaussian", nlambda=1, alpha=.5, lambda=0.05)


#fit corresponding H2O model

fitH2O<- h2o.glm.FV(x=c("Height", "Volume"), y="Girth", family="gaussian", nfolds=0, alpha=.5, lambda=0.05, data=treesH2O)
...
doTest("GLM Test: Regularization: Alpha=.5, verif with penalized", test.glm2glmnetreg3.golden)

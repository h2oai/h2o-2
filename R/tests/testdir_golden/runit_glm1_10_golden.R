...
test.glm1glmnetregtrees2.golden <- function(H2Oserver) {
	
#Import data: 
Log.info("Importing TREES data...") 
treesH2O<- h2o.uploadFile.VA(H2Oserver, locate("../smalldata/trees.csv"), key="treesH2O.VA")
...
fitRglmnet<- glmnet(x, y, family="gaussian", nlambda=1, alpha=.3, lambda=0.04010316)


#fit corresponding H2O model

fitH2O<- h2o.glm.VA(x=c("Height", "Volume"), y="Girth", family="gaussian", nfolds=0, alpha=.3, lambda=0.04010316, data=treesH2O)
...
doTest("GLM1 Test: Regularization: Alpha=.3", test.glm1glmnetregtrees2.golden)

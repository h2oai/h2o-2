...
test.createFrame <- function(conn) {
  ...
  hex <- h2o.createFrame(conn, "hex", rows = 1000, cols = 20, randomize = TRUE, categorical_fraction = 0.1, factors = 5, integer_fraction = 0.5, integer_range = 1)
  ...
  hex2 <- h2o.createFrame(conn, "hex2", rows = 1000, cols = 20, randomize = FALSE, value = 5, categorical_fraction = 0, integer_fraction = 0, missing_fraction = 0, has_response = TRUE)
  print(summary(hex2))

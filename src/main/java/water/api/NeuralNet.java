...
public class NeuralNet extends Request {
  ...
  protected final H2OExistingKey _description = new H2OExistingKey(DESCRIPTION);
  ...
  static class Builder extends ObjectBuilder {
    ...
    private void modelHTML(KMeansModel m, JsonObject json, StringBuilder sb) {
      ...
      sb.append("<div class='alert'>Actions: " + KMeansScore.link(m._selfKey, "Score on dataset") + ", "
          + KMeansApply.link(m._selfKey, "Apply to dataset") + ", " + NeuralNet.link(m._dataKey, "Compute new model")
          + "</div>");

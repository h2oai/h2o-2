...
public abstract class PersistHdfs {
  ...
  public static void cleanIce(URI dir) {
    try {
      FileSystem fs = FileSystem.get(dir, CONF);
  ...
  static Key getKeyForPathString(String str) {
    ...
    assert getPathStringForKey_impl(key).equals(str) : "hdfs name bijection: '" + str + "' makes key " + key
        + " makes '" + getPathStringForKey_impl(key) + "'";
    return key;
  }

  static private String getPathStringForKey(Key key) {
    String str = getPathStringForKey_impl(key);
    assert getKeyForPathString_impl(str).equals(key) : "hdfs name bijection: key " + key + " makes '" + str
        + "' makes key " + getKeyForPathString_impl(str);
    return str;
  }

  // Actually we typically want a Path not a String
  public static Path getPathForKey(Key k) {
    return new Path(getPathStringForKey(k));
  }
  ...
  public static byte[] fileLoad(Value v) {
    return fileLoad(v, null);
  }

  public static byte[] fileLoad(Value v, String path) {
    ...
    Path p = path != null ? new Path(path) : getPathForKey(k);
  ...
  public static void mkdir(URI uri) {
    try {

  ...
  public static class ParallelGLMs extends H2OCountedCompleter {
    ...
    class Callback extends H2OCallback<H2OCountedCompleter> {
      ...
      @Override public void callback(H2OCountedCompleter cc){
        System.out.println("Callback " + this + " done, completer = " + getCompleter());
        if(!_tasks[i]._done)
          System.out.println("gaga");
        assert _tasks[i]._done:" onCompletion but task is not done!";
        int i;
    ...
    @Override public void onCompletion(CountedCompleter cc){
      for(int i= 0; i < _tasks.length; ++i) {
        assert _tasks[i]._done:i + ": GLM2[dest=" + _tasks[i]._glm.dest() +"]: pgs claims to be done before all tasks finished!";

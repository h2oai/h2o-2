...
# CDH4_YARN_JAR_USED=$H2O_DOWNLOADED/hadoop/$CDH4_YARN_JAR
# CDH4_YARN_JAR_USED=$H2O_DOWNLOADED/$CDH4_YARN_JAR
H2O_JAR_USED=$H2O_BUILT/$H2O_JAR
CDH4_YARN_JAR_USED=$H2O_BUILT/hadoop/$CDH4_YARN_JAR
...
echo "$SET_JAVA_HOME" > /tmp/h2o_on_hadoop_$REMOTE_IP; chmod 777 /tmp/h2o_on_hadoop_$REMOTE_IP.sh
...
$REMOTE_SCP $CDH4_YARN_JAR_USED $REMOTE_USER:$REMOTE_HOME
...
../find_cloud.py -f h2o_one_node -hdfs_version hdp2.1 -hdfs_name_node $NAME_NODE -expected_size $CDH4_YARN_NODES

...
class Basic(unittest.TestCase):
    ...
    def test_parse_rand_utf8_unmatched_double(self):
        ...
        for (rowCount, colCount, hex_key, timeoutSecs) in tryList:
            ...
            if DOUBLE_QUOTE:
                self.assertTrue((numCols==colCount or numCols==colCount+1), colCount, msg='Wrong numCols: %s %s' % (numCols, colCount))
            else:
                self.assertTrue(numCols==colCount, colCount, msg='Wrong numCols: %s %s' % (numCols, colCount))

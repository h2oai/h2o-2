...
dw_3:
	...
	sed -i -e "s/SUBST_RELEASE_NAME/$(RELEASE_NAME)/g" $(BUILD_WEBSITE_DIR)/deployment/hadoop_tutorial.html
	sed -i -e "s/SUBST_PROJECT_VERSION/$(PROJECT_VERSION)/g" $(BUILD_WEBSITE_DIR)/deployment/hadoop_tutorial.html
	sed -i -e "s/SUBST_BUILD_NUMBER/$(BUILD_NUMBER)/g" $(BUILD_WEBSITE_DIR)/deployment/hadoop_tutorial.html

...
gID         <- paste(paste("cpu_", paste(pID, args[2], sep="_"),sep=""), ".svg", sep="")
...
CPU         <- read.csv(args[1])
mint        <- min(CPU$time.s.)
CPU$time.s. <- CPU$time.s. - mint
aspect      <- 1/12
trellis.par.set(layout.widths = list(left.padding = 10))
trellis.par.set(par.main.text = list(alpha = 0.2))
my.plot     <- xyplot(cpu ~ time.s.,
                  aspect   = aspect,
                  data     = CPU,
                  type     = "a",
                  main     = paste(toupper(pID), paste("Aggregate CPU Utilization on ", mID, sep=""),sep=" "),
                  xlab     = "time(s)",
                  ylab     = "CPU Utilization % (32 cores)",
                  ylim     = c(-1,100),
                  xlim     = c(min(CPU$time.s.),max(CPU$time.s.)))
...
labels     <- c("EndParseAir1x",    paste("End",paste(toupper(args[3]),"Air1x",   sep=""), sep=""), "EndParseAir10x",  paste("End",paste(toupper(args[3]), "Air10x",  sep=""), sep=""),
   ...
   x3      <- interleave(x1,x2)
   cuts    <- cumsum(x3)
   cuts    <- cuts + min(CPU$time.s.)
   print("CUUUUUUUUUUUUUTS")
   print(cuts)
   print("CUUUUUUUUUUUUUTS")
   makeCuts(cuts,pID)
...
#uncomment this to get all cpus
#print(trellis.par.get())
#for(i in names(CPU[,-c(1,2)])) {
#  trellis.focus("panel",1,1)
#  panel.lines(CPU$time.s.,CPU[i][,1])
#  trellis.unfocus()
#}

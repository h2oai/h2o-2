...
csv_header = ('java_heap_GB','dataset','nTrainRows','nTestRows','nCols','trainParseWallTime','nfolds','glmBuildTime','testParseWallTime','scoreTime','AUC','AIC','error','AverageErrorOverNFolds')
...
if __name__ == '__main__':
    ...
    testFile                    = h2i.import_parse(bucket='home-0xdiag-datasets', path='bench-test/Airlines/AirlinesTest.csv', schema='local', hex_key="atest.hex",timeoutSecs=3600,retryDelaySecs=5, pollTimeoutSecs=3600)
    elapsedAirlinesTestParse    = time.time() - airlinesTestParseStart
    
    row = {'testParseWallTime' : elapsedAirlinesTestParse}
    doGLM(files['Airlines'], 'Airlines', 'binomial', 'logit', 1E-5, 0.5, 10, 'IsDepDelayed', 'Year,Month,DayofMonth,DayOfWeek,DepTime,ArrTime,UniqueCarrier,Origin,Dest,Distance',testFile['destination_key'],row)
    ...
    testFile                    = h2i.import_parse(bucket='home-0xdiag-datasets', path='bench-test/AllBedrooms/AllBedroomsTest.csv', schema='local', hex_key="allBtest.hex",timeoutSecs=3600,retryDelaySecs=5, pollTimeoutSecs=3600)

...
function Perfbar(nodeName, nodeIdx, numCores) {
    ...
    this.docWriteCanvas = function() {
        ...
        if (numCores == 0) {
            ...
            width = (8 * PB_PIXEL_WIDTH_BAR) + ((numCores - 1) * PB_PIXEL_WIDTH_SPACING);
        }
        else {
            width = (numCores * PB_PIXEL_WIDTH_BAR) + ((numCores - 1) * PB_PIXEL_WIDTH_SPACING);
        }

        // Minimum width for the "node 0" string to print properly.
        var min_width = 35;
        if (width < min_width) {
            width = min_width;
        }
        ...
        var height = (PB_PIXEL_HEIGHT + PB_PIXEL_HEIGHT_SPACING + PB_PIXEL_HEIGHT_LINEOFTEXT);
    ...
    this.updateTicks = function(cpuTicks) {
        if (this.pbFirstUpdate) {
            ...
            this.pbCtx.fillRect(
                ...
                PB_PIXEL_HEIGHT,
                this.pbWidth,
                PB_PIXEL_HEIGHT_SPACING + PB_PIXEL_HEIGHT_LINEOFTEXT);
...
function initializeNode(nodeIdx) {
    ...
    xmlhttp.open("GET","/2/WaterMeterCpuTicks.json?node_idx=" + nodeIdx,true);
    xmlhttp.send();
}

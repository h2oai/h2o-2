...
//        maxChunkSize: 1000, // uncomment to enable chunked uploads - supported only in FF&Chrome
          ...
          $('#UploadBtn').unbind('click').click(function(e) {    	
        	...
        	if ($.isArray(result)) { 
        		if (result[0]) {
                	...
                	window.location.href='Inspect?Key=' + encodeURIComponent(result.key);
                	return;
        		}
        	} 
    ...
    function resetUploadTable() {
    	$('#UploadTable').empty();
    	$('#UploadTable').append("<tr><td class='span1' id='btntd'><button type='submit' class='btn btn-primary' id='UploadBtn'>Upload</button></td><td class='span2' id='filename'></td>"

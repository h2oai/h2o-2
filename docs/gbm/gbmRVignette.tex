...
Download H2O
{\href{http://www.h2o.ai/download} {\underbar{http://www.h2o.ai/download}}

Join the Community
{\href{h2ostream@googlegroups.com} {\underbar{h2ostream@googlegroups.com}} and {\href{github.com/h2oai/h2o.git}{\underbar{github.com/h2oai/h2o.git}}


\section{Introduction}

This vignette presents the gradient boosted machine (GBM) framework in the H2O package at {\href{http://cran.r-project.org/web/packages/h2o/index.html}{\underbar{http://cran.r-project.org/web/packages/h2o/index.html}}}. Further documentation on H2O's system and algorithms can be found at the h2o.ai website at {\href{http://docs.h2o.ai}{\underbar{http://docs.h2o.ai}}} and the R package manual at {\href{http://cran.r-project.org/web/packages/h2o/h2o.pdf}{\underbar{http://cran.r-project.org/web/packages/h2o/h2o.pdf}}}. The full datasets and code of this vignette can be found at the H2O Github at {\href{github.com/h2oai/h2o.git}{\underbar{github.com/h2oai/h2o.git}}}. This introductory section provides instructions on getting H2O started, followed by a brief overview of gradient boosting.
...
\subsection{Support} 
Users of the H2O package may submit general enquiries and bug reports to h2o.ai support at {\href{h2ostream@googlegroups.com} {\underbar{h2ostream@googlegroups.com}}. Alternatively, specific bugs or issues may be filed to the h2o.ai JIRA at {\href{https://0xdata.atlassian.net/secure/Dashboard.jspa}{\underbar{https://0xdata.atlassian.net/secure/Dashboard.jspa}}.
...
\indent For $k=1$ to $K$
\\
\indent  Compute $r_{ikm} = y_{ik} - p_k(x_i), \hspace{5pc} i = 1,2,\dots,N$
\\
\indent \indent Fit a regression tree to the targets $r_{ikm}, \hspace{5pc} i = 1,2,\dots,N$, giving terminal regions $R_{jim}, \hspace{5pc} 1,2,\dots,J_m$
\\
\indent \indent Compute $\gamma_{jkm} = \frac{K-1}{K} \frac{\sum_{x_i \in R_{jkm}} (r_{ikm}}{\sum_{x_i \in R_{jkm}} |r_{ikm}| (1 - |r_{ikm}|)} , \hspace{5pc} j=1,2,\dots,J_m$
\\
\indent \indent Update $f_{km}(x) = f_{k,m-1}(x) + \sum_{j=1}^{J_m} \gamma_{jkm} I(x \in R_{jkm})$
\\
Output $f_k^{\hat{}}(x) = f_{kM}(x), \hspace{5pc} k=1,2,\dots,K$
\\
\line(1,0){500}
\\
\\ 
In the above algorithm, the index $m$ tracks of the number of weak learners added to the current ensemble. Within this outer loop, there is an inner loop across each of the $K$ classes. In this inner loop, the first step is to compute the residuals, $r_{ikm}$, which are actually the gradient values, for each of the $N$ bins in the CART model, and then to fit a regression tree to these gradient computations. This fitting process is distributed and parallelized, and details on this framework can be found on the h2o.ai blog at \newline{\href{http://h2o.ai/blog/2013/10/building-distributed-gbm-h2o/}{\underbar{http://h2o.ai/blog/2013/10/building-distributed-gbm-h2o/}}.
\\
\\
...
Download the Airline dataset from:\newline {\href{https://github.com/h2oai/h2o/blob/master/smalldata/airlines/allyears2k_headers.zip}{\underbar{https://github.com/h2oai/h2o/blob/master/smalldata/airlines/allyears2k\_headers.zip}} and save the .csv file to your working directory. Before running the Airline demo, review how to load data with H2O. 

\subsubsection{Loading data}  

Loading a dataset in R for use with H2O is slightly different from the usual methodology, as we must convert our datasets into \texttt{H2OParsedData} objects. For this example, download the toy weather dataset from {\href{https://raw.githubusercontent.com/0xdata/h2o/master/smalldata/weather.csv}{\underbar{https://raw.githubusercontent.com/0xdata/h2o/master/smalldata/weather.csv}}.  Load the data to your current working directory in your R Console (do this for any future dataset downloads), and then run the following command.
\begin{spverbatim}

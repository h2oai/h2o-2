test 'FrameListView: should display all frames when no predicate is applied', (t) ->
  ...
  link$ _.framesLoaded, ->
    t.equal frameListView.items().length, 4, 'frameListView.items() array lengths match'
    ...
    t.equal frameListView.items()[2].title, 'has_uuid.hex', 'String frameListView.items()[2].title equals [has_uuid.hex]'
    t.equal frameListView.items()[2].caption, '89 columns: Randm, Month, applicationid, zip, sex', 'String frameListView.items()[2].caption equals [89 columns: Randm, Month, applicationid, zip, sex]'
